...
static int hwts_set(int fd, const char *device, int rx_filter, int rx_filter2,
		    int tx_type)
{
	struct ifreq ifreq;
			   ...
			   bool filter_event_supported)
{
	...
	err = sk_ts_get_rx_filter(type, transport, is_master,
				  filter_event_supported, &filter1, &filter2);
	if (err)
		return err;
...
int sk_timestamping_init(int fd, const char *device, enum timestamp_type type,
			 enum transport_type transport, bool filter_event_supported)
{
